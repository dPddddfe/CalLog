...
// 이미 여기서 import 되었으므로, 하단에서 재선언할 필요 없습니다.
...
const TodayDietPage = () => {
  ...
  // 저장된 단식 기록 불러오기 (초 단위로 저장된 ms 값 사용)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (isFasting) {
      ...
      // if stopped, fastElapsed already set to final value and saved on stop
    }
  ...
  const stopFasting = () => {
    setIsFasting(false);
    // fastElapsed currently holds elapsed ms
    localStorage.setItem("fastRecord", String(fastElapsed));
    localStorage.setItem("fastRunning", 'false');
    // keep fastStartTime in storage if you want history; do not clear it here
  };
  ...
  return (
    <div className="today-diet-layout">
      ...
      <div className="left-panel">
        {/* 상단: 요약 카드들 - 순서 변경 및 세로 배열을 위해 클래스 변경 */}
...
/*
* ERROR 발생 지점:
* 상단에서 import WeeklyCalendarPage, MonthlyCalendarPage, CalorieManagementPage 를 했기 때문에,
* 하단에서 'const CalorieManagementPage = ...' 처럼 다시 선언하면 충돌이 발생합니다.
* 따라서 아래 코드는 삭제하거나 주석 처리해야 합니다.

const CalorieManagementPage = () => <div className="placeholder-page-wrapper">칼로리 관리 페이지</div>;
const WeeklyCalendarPage = () => <div className="placeholder-page-wrapper">주간 달력 페이지</div>;
const MonthlyCalendarPage = () => <div className="placeholder-page-wrapper">월간 달력 페이지</div>;
*/
...
const App = () => {
  ...
  const styles = `
    ...
    .summary-section-full-width { /* 전체 너비를 사용 */
      flex: none; 
      width: 100%;
      display: flex;
      flex-direction: column;
